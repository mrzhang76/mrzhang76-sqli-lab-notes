# Less-19 **POST Header Injection- Referer- Error Based- string**
TAG：POST注入、HTTP头注入、报错注入  
## 漏洞利用  
本less类似于less18，但是要在referer中引入恶意语句，页面会回显用户请求头中携带的referer    
![less19_1](images\less19_1.png)  

使用burpsuite修改请求头，在referer中加入恶意语句来获取数据库名  
payload:```' and updatexml(1,concat(0x7e,(select @@version),0x7e),1) and '1'='1```
![less19_2](images\less19_2.png)  

恶意语句成功注入，数据库名被回显  
![less19_3](images\less17_3.png)  
注意，这个注入也需要在登陆状态下才能正常触发
...
```
### 漏洞点分析  
此页面的漏洞和less18的漏洞很相似，只是写入数据库的为请求头中携带的referer  
```
if($row1){
	echo '<font color= "#FFFF00" font size = 3 >';
	$insert="INSERT INTO `security`.`referers` (`referer`, `ip_address`) VALUES ('$uagent', '$IP')";
